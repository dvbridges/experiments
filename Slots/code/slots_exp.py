...
"""
# Set condition file
conditions=pd.read_csv("conditions/conds_use.csv", delimiter=',')
...
SAVE_FOLDER = 'RB_Data'  # Log is saved to this folder. The folder is created if it does not exist.
...
def setTarget(trial_num):
    ...
    #Loss changes
    #Numbers for Right block
    if trials.iloc[trial_num]['Condition']=='Loss' and trials.iloc[trial_num]['CB']==2:
        right[0]=trials.iloc[trial_num]['Right']+1

    if trials.iloc[trial_num]['Condition']=='Loss' and trials.iloc[trial_num]['CB']==1:
        left[0]=trials.iloc[trial_num]['Left']+1
...
data.to_csv('resp_data/'+V['subject']+'_'+time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime()) +'.csv', delimiter=',', columns=[
