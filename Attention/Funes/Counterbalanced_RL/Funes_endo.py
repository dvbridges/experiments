...
V = {'subject':'', 'condition': ['en'], 'age':'', 'gender':['male', 'female']}
...
TARG_POSITIONS = [-5,5]
TARG_DIRECTION = [-5,5]
REPETITIONS = 6 # 16 trials per rep (8 are congruent) gives 64 trials per block
...
mainBegin=visual.TextStim(win, text=breaktxt3, pos=([0,0]), height = 1, wrapWidth = 30)
...
def run_condition(condition):
	...
	for trial in make_trial_list(condition):
		...
		blank_screen(False)
		# Set cue color
		color=setCueColor(trial['cue_pos'])
		cue.fillColor = color
		cue.lineColor = color
